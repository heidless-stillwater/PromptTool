...
function ExemplarItem({ entry }: { entry: CommunityEntry }) {
    ...
    return (
        <Link
            href={`/generate?exemplarId=${entry.id}`}
        ...
        >
            <div className="aspect-[3/4] rounded-2xl overflow-hidden relative mb-3 border border-white/10 shadow-sm group-hover:shadow-2xl group-hover:shadow-primary/20 group-hover:border-primary/50 transition-all duration-500 transform group-hover:-translate-y-1">
                ...
                <div className="absolute top-3 left-3 z-10 bg-primary text-white p-1.5 rounded-lg shadow-lg border border-white/10 transform -rotate-12 group-hover:rotate-0 transition-transform duration-300">
                    <Icons.star size={12} className="fill-white" />
                </div>
            </div>
            <div className="px-1">
...
export default function Exemplars({ entries }: ExemplarsProps) {
    return (
        <div className="mb-14 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center border border-primary/20 shadow-lg shadow-primary/5">
                        <Icons.star size={20} className="text-primary fill-primary animate-pulse" />
                    </div>
                    <div>
                        <h2 className="text-xl font-black uppercase tracking-widest text-white/90">Exemplars</h2>
                        <p className="text-[10px] uppercase tracking-widest text-white/50 font-bold mt-1">Hand-picked Masterpieces</p>
                    </div>
                </div>
                <Link href="/generate">
            ...
            <div className="flex gap-6 overflow-x-auto pb-6 scrollbar-hide snap-x -mx-4 px-4 mask-fade-right">
                {entries.length > 0 ? (
                    entries.map((entry) => (
                        <ExemplarItem key={entry.id} entry={entry} />
                    ))
                ) : (
                    <div className="w-full py-12 flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-3xl bg-white/5">
                        <Icons.sparkles className="text-white/20 mb-2" size={32} />
                        <p className="text-sm font-bold text-white/40 uppercase tracking-widest">No Exemplars Curated Yet</p>
                        <p className="text-[10px] text-white/30 mt-1 uppercase tracking-widest">Admin-approved gallery pieces will appear here.</p>
                    </div>
                )}
            </div>
        </div>
