// Admin League Backfill API — Re-syncs all league entries with fresh source image + author data
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (totalEntries === 0) {
            return NextResponse.json({ success: true, message: 'No league entries found', updated: 0, total: 0 });
        }
                ...
                if (originalUserId && originalImageId) {
                    try {
                        ...
                        if (imageDoc.exists) {
                            ...
                            // Sync videoUrl if missing on league entry
